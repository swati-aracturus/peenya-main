...
const cards = [
  ...
  const CardSwiper = () => {
    ...
    const maxIndexMobile = Math.ceil(totalCards / cardsToShowMobile) - 1;

    const goToPrevious = () => {
      setCurrentIndex((prevIndex) =>
        prevIndex > 0 ? prevIndex - 1 : (window.innerWidth < 768 ? maxIndexMobile : maxIndex)
      );
    };

    const goToNext = () => {
      setCurrentIndex((prevIndex) =>
        prevIndex < (window.innerWidth < 768 ? maxIndexMobile : maxIndex) ? prevIndex + 1 : 0
      );
    };
      ...
      <div className="relative w-[90%] py-8 mx-auto">
        <div className="overflow-hidden w-full">
          <div
            ...
            style={{
              transform: `translateX(-${currentIndex * (100 / (window.innerWidth < 768 ? cardsToShowMobile : cardsToShow))}%)`,
            }}
          >
            {cards.map((card) => (
              <div
                key={card.id}
                className={`flex-shrink-0 p-2  ${window.innerWidth < 768 ? 'w-full' : 'w-[326px]'}`}
              >
                <div className="relative rounded-2xl overflow-hidden text-white h-[490px] cursor-pointer ">
                  <img
                    src={card.image}
                    alt={card.city}
                    className="object-cover w-full h-full transition-transform duration-300 ease-in-out hover:scale-110"
                  />
                  <div className="absolute inset-0 flex flex-col justify-end p-8 bg-black bg-opacity-50 transition-transform duration-300 ease-in-out hover:scale-110">
                    <div>
